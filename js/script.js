...
$(document).ready(function () {
    $(document).on('click', '.sumBlock__btn', function (e) {
        e.preventDefault();
    })
    //меню
    ...
    if ($('.regimeToggler').length > 0) {
        $(document).on('click', '.regimeToggler', function (e) {
            e.preventDefault();
    ...
    if ($('.js-range-slider').length > 0) {
        ...
        //изменение textinput
        $(document).on('change', '.specificationBlock__amount:not(.specificationBlock__amount_custom):not(.specificationBlock__amount_diskIOPS) ', function () {
            let value = $(this).val();
    ...
    if ($('.duplicateBtn').length > 0) {
        $(document).on('click', '.duplicateBtn', function (e) {
            ...
            elem.find('.specificationBlock').eq(0).append('<a href="#" class="removeDisk"></a>');
        ...
        $(document).on('click', '.removeDisk', function (e) {
            e.preventDefault();
            $(this).closest('.diskBlock').remove();
            if ($('.configurator').length > 0) {
                configuratorProvidersCheck();
                configuratorCalc();
            }
    ...
    if ($('.configurator').length > 0) {
        ...
        $(document).on('change', '.specificationBlock__amount_diskIOPS', function () {
            let val = $(this).val();
            let result = '';
            for (const [key, value] of Object.entries(diskTypeSettings)) {
                if (+value[0] >= +val) {
                    result = key;
                    break;
                }
            ...
            $(this).closest('.specificationBlock').find('input[value="' + result + '"]').prop('checked', true);
        });
        ...
        function configuratorCalc() {
            ...
            if (($('.rw').length > 0) && (!$('.regimeToggler').hasClass('active')) && ($('.rw-description__block_vdi').hasClass('rw-description__block_inactive'))) {
                ...
                $('.sumBlock__row.remoteBasePrice .sumblock-row__text').html('Стандартное удаленное рабочее место');
                $('.sumBlock__row.remoteBasePrice .sumblock-row__quantity').html(placesAmount);
                $('.sumBlock__row.remoteBasePrice .sumblock-row__single').html(remoteBasePrice * remoteTypeCoef);
                ...
                var diskText = 'Стандартное сетевое хранилище';
                ...
                var sum = (vCPUBasePrice * vcpuAmount + vRAMGBBasePrice * VRAMAmount + vGPUBasePrice * vGPUAmount + disksSum + disksIOPSSum) * placesAmount * remoteTypeCoef + extIPBasePrice * extIPAmount + speedPrice[speedAmount] * guaranteedSpeed;

                $('.sumBlock__row.vCPUBasePrice .sumblock-row__text').html('vCPU');

                $('.sumBlock__row.vCPUBasePrice .sumblock-row__quantity').html(vcpuAmount * placesAmount);

                $('.sumBlock__row.vCPUBasePrice .sumblock-row__single').html(vCPUBasePrice * remoteTypeCoef);
                $('.sumBlock__row.vCPUBasePrice .sumblock-row__price').html((vCPUBasePrice * vcpuAmount * placesAmount * remoteTypeCoef).toFixed(2) + ' Р');

                $('.sumBlock__row.vRAMGBBasePrice .sumblock-row__text').html('RAM');
                $('.sumBlock__row.vRAMGBBasePrice .sumblock-row__quantity').html(VRAMAmount * placesAmount);
                $('.sumBlock__row.vRAMGBBasePrice .sumblock-row__single').html(vRAMGBBasePrice * remoteTypeCoef);
                ...
                if (vGPUAmount >= 0) {
                    $('.sumBlock__row.vGPUBasePrice .sumblock-row__text').html('VGPU');
                    $('.sumBlock__row.vGPUBasePrice .sumblock-row__quantity').html(vGPUAmount * placesAmount);
                    $('.sumBlock__row.vGPUBasePrice .sumblock-row__single').html(vGPUBasePrice * remoteTypeCoef);
                ...
                $('.sumBlock__row.disksSum .sumblock-row__quantity').html(disksGB);
                $('.sumBlock__row.disksSum .sumblock-row__single').html(diskGbBasePrice);
                $('.sumBlock__row.disksSum .sumblock-row__price').html(((disksSum + disksIOPSSum) * placesAmount * remoteTypeCoef).toFixed(2) + ' Р');

                $('.sumBlock__row.extIPBasePrice .sumblock-row__text').html('Внешний IP');
                $('.sumBlock__row.extIPBasePrice .sumblock-row__quantity').html(extIPAmount);
                $('.sumBlock__row.extIPBasePrice .sumblock-row__single').html(extIPBasePrice);
                ...
                if (guaranteedSpeed != 0) {
                    ...
                    $('.sumBlock__row.speedPrice .sumblock-row__price').html((speedPrice[speedAmount] * guaranteedSpeed).toFixed(2) + ' Р');
                    $('.sumBlock__row.speedPrice').slideDown(300);
                    $('.sumBlock__row.speedPrice').removeClass('slided');
                } else {
                    $('.sumBlock__row.speedPrice').slideUp(300);
                    $('.sumBlock__row.speedPrice').addClass('slided');
                }
        ...
        function cybernetsCalc() {
            ...
            $('.diskBlock').each(function () {
                if ($(this).find('[type="radio"]:checked').val() == 'hdd') {
                    diskHDDCount = +diskHDDCount + parseInt($(this).find('.specificationBlock__amount_disk').val());
                    HDDSum = HDDSum + $(this).find('.specificationBlock__amount_disk').val() * diskGbBasePrice * diskTypeCoefficient[$(this).find('[type="radio"]:checked').val()];
                } else {
                    diskSSDCount = +diskSSDCount + parseInt($(this).find('.specificationBlock__amount_disk').val());
                    SSDSum = SSDSum + $(this).find('.specificationBlock__amount_disk').val() * diskGbBasePrice * diskTypeCoefficient[$(this).find('[type="radio"]:checked').val()];
                }
            });
            var hostAmount = 0;
            if ($('[name="masterType"]:checked').val() == 'regional') {
            ...
            var sum = (basePrice + masterTypePrice[masterType] + vCPUBasePrice * vcpuAmount * vCPUShareCoefficient[vCPUShare] + vRAMGBBasePrice * VRAMAmount + HDDSum + SSDSum + hostAmount * hostBasePrice) * periodCoefficient[period];

            $('.sumBlock__row.basePrice .sumblock-row__text').html('Kubernetes®');
            $('.sumBlock__row.basePrice .sumblock-row__quantity').html('1');
            $('.sumBlock__row.basePrice .sumblock-row__single').html((basePrice * periodCoefficient[period]).toFixed(2));
            ...
            $('.sumBlock__row.masterTypePrice .sumblock-row__quantity').html('1');
            $('.sumBlock__row.masterTypePrice .sumblock-row__single').html((masterTypePrice[masterType] * periodCoefficient[period]).toFixed(2));
            $('.sumBlock__row.masterTypePrice .sumblock-row__price').html((masterTypePrice[masterType] * periodCoefficient[period]).toFixed(2) + ' Р');

            $('.sumBlock__row.vCPUBasePrice .sumblock-row__text').html(vCPUShare + 'vCPU');
            $('.sumBlock__row.vCPUBasePrice .sumblock-row__quantity').html(vcpuAmount);
            $('.sumBlock__row.vCPUBasePrice .sumblock-row__single').html(vCPUBasePrice * vCPUShareCoefficient[vCPUShare] * periodCoefficient[period]);
            ...
            $('.sumBlock__row.vRAMGBBasePrice .sumblock-row__text').html('RAM');
            $('.sumBlock__row.vRAMGBBasePrice .sumblock-row__quantity').html(VRAMAmount);
            $('.sumBlock__row.vRAMGBBasePrice .sumblock-row__single').html(vRAMGBBasePrice * periodCoefficient[period]);
            $('.sumBlock__row.vRAMGBBasePrice .sumblock-row__price').html((vRAMGBBasePrice * VRAMAmount * periodCoefficient[period]).toFixed(2) + ' Р');

            if (diskHDDCount > 0) {
                $('.sumBlock__row.HDDSum').slideDown(300);
                $('.sumBlock__row.HDDSum').removeClass('slided');
                $('.sumBlock__row.HDDSum .sumblock-row__text').html(diskTypeName['hdd']);
                $('.sumBlock__row.HDDSum .sumblock-row__quantity').html(diskHDDCount);
                $('.sumBlock__row.HDDSum .sumblock-row__single').html(diskGbBasePrice * diskTypeCoefficient['hdd']);
                $('.sumBlock__row.HDDSum .sumblock-row__price').html((HDDSum * periodCoefficient[period]).toFixed(2) + ' Р');
            } else {
                $('.sumBlock__row.HDDSum').slideUp(300);
                $('.sumBlock__row.HDDSum').addClass('slided');
            }
            if (diskSSDCount > 0) {
                $('.sumBlock__row.SSDSum').slideDown(300);
                $('.sumBlock__row.SSDSum').removeClass('slided');
                $('.sumBlock__row.SSDSum .sumblock-row__text').html(diskTypeName['ssd']);
                $('.sumBlock__row.SSDSum .sumblock-row__quantity').html(diskSSDCount);
                $('.sumBlock__row.SSDSum .sumblock-row__single').html(diskGbBasePrice * diskTypeCoefficient['ssd']);
                $('.sumBlock__row.SSDSum .sumblock-row__price').html((SSDSum * periodCoefficient[period]).toFixed(2) + ' Р');
            } else {
                $('.sumBlock__row.SSDSum').slideUp(300);
                $('.sumBlock__row.SSDSum').addClass('slided');
            }

            if (hostAmount != 0) {
                $('.hostBasePrice').slideDown(300);
                $('.hostBasePrice').removeClass('slided');
                $('.sumBlock__row.hostBasePrice .sumblock-row__text').html('Хост');
                $('.sumBlock__row.hostBasePrice .sumblock-row__quantity').html(hostAmount);
                $('.sumBlock__row.hostBasePrice .sumblock-row__single').html(hostBasePrice * periodCoefficient[period]);
